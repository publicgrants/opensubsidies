...
import { Flame, Trophy, Target, TrendUp, Plus } from "@phosphor-icons/react";
      ...
      {habits.length === 0 ? (
        ...
        <Tabs defaultValue={habits[0]?.id ?? ""} className="gap-0 flex flex-col">
          <div className="mb-5">
            ...
            <TabsList className="h-auto w-full gap-1 bg-transparent p-0 flex-nowrap overflow-x-auto justify-start pb-1 scrollbar-none">
              {habits.map((habit) => (
                <TabsTrigger
                  key={habit.id}
                  value={habit.id}
                  className="flex-none h-auto gap-1.5 data-active:bg-card data-active:shadow-sm data-active:after:opacity-0 rounded-lg shrink-0"
                >
